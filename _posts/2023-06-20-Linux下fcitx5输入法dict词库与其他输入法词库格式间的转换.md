...
原始词库文件是一个文本文件（如搜狗、qq、百度拼音输入法都支持txt格式的词库），其格式为： `汉字 拼音 频率`
...
如果在其他输入法软件中导出或下载的词库不是txt格式，但又想把词库导入fcitx5使用，可以先使用[深蓝词库转换工具](https://github.com/studyzy/imewlconverter)把相关词库转换为txt格式，然后再按照`方法1`转换为fcitx5可以使用的txt格式。
